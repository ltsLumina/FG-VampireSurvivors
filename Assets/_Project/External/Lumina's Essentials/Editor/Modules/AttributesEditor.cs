                    ...
                    if (validRange.rangeDisplayType != RangedFloatAttribute.RangeDisplayType.HideRanges)
                    {
                        ...
                        switch (validRange.rangeDisplayType)
                        {
                            case RangedFloatAttribute.RangeDisplayType.LockedRanges:
                                currentMinCopy = EditorGUI.FloatField(lower, $"Lower (Min: {validRange.min.ToString(CultureInfo.InvariantCulture)})", currentMinCopy);

                                currentMaxCopy = EditorGUI.FloatField(upper, $"Upper (Max: {validRange.max.ToString(CultureInfo.InvariantCulture)})", currentMaxCopy);
